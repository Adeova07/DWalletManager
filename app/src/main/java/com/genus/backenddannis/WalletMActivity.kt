...
class WalletMActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val transferList: List<Transfer> = appDatabase.transferDao().getAll() // Ambil daftar transfer dari database
            val kategoriDao = appDatabase.kategoriDao()

            withContext(Dispatchers.Main) {
                // Inisialisasi AdapterWallet dan atur RecyclerView dengan adapter
                val adapter = AdapterWallet(this@WalletMActivity, kategoriDao, pendapatanList, pengeluaranList, transferList)
