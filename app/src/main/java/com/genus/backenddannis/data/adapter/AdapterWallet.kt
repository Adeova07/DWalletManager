...
class AdapterWallet(
    ...
    private val transferList: List<Transfer>
    ...
    init {
        // Mengambil kategori dari database dan menggabungkannya dengan kategori dari pendapatan, pengeluaran, dan transfer
        CoroutineScope(Dispatchers.IO).launch {
            val kategoriFromDb = kategoriDao.getKategoriAddList()
            uniqueKategoriDompetList = (kategoriFromDb +
                    pendapatanList.map { it.kategoriDompet } +
                    pengeluaranList.map { it.kategoriDompet } +
                    transferList.map { it.kategoriDari } +
                    transferList.map { it.kategoriKepada }).distinct()
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == VIEW_TYPE_KATEGORI) {
            ...
            val totalPengeluaran = pengeluaranList.filter { it.kategoriDompet == kategoriDompet }
                .sumOf { it.jumlah }
            val totalTransferKeluar = transferList.filter { it.kategoriDari == kategoriDompet }
                .sumOf { it.jumlahT }
            val totalTransferMasuk = transferList.filter { it.kategoriKepada == kategoriDompet }
                .sumOf { it.jumlahT }

            val saldo = totalPendapatan - totalPengeluaran + totalTransferMasuk - totalTransferKeluar
            ...
            walletHolder.itemView.findViewById<View>(R.id.CdContent).setOnClickListener {
                val intent = Intent(context, DetailSaldo::class.java).apply {
                    ...
                    putExtra("TRANSFER_KELUAR_LIST", ArrayList(transferList.filter { it.kategoriDari == kategoriDompet }) as Serializable)
                    putExtra("TRANSFER_MASUK_LIST", ArrayList(transferList.filter { it.kategoriKepada == kategoriDompet }) as Serializable)
                }
